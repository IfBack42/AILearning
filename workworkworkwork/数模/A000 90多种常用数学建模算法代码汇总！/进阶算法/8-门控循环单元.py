...
# 将数据扩展为 3D 形状，以符合GRU网络的输入要求
X_scaled = np.expand_dims(X_scaled, axis=2)  # 扩展数据的维度，将其从2D变为3D，以适应GRU输入格式
...
# 4. 定义门控循环单元 (GRU) 网络
def build_gru(input_shape):  # 定义GRU网络的构建函数
    inputs = layers.Input(shape=input_shape)  # 定义输入层，输入形状为3D
    x = layers.GRU(128, activation='tanh', return_sequences=True)(inputs)  # 第一个GRU层，128个单元，返回序列
    x = layers.GRU(64, activation='tanh', return_sequences=False)(x)  # 第二个GRU层，64个单元，不返回序列
    x = layers.Dense(64, activation='relu')(x)  # 全连接层，64个单元，ReLU激活函数
    x = layers.Dropout(0.3)(x)  # 添加Dropout层，防止过拟合
    outputs = layers.Dense(1, activation='linear')(x)  # 最后接一个全连接层，进行线性回归
    model = Model(inputs=inputs, outputs=outputs)  # 构建模型
    return model  # 返回构建的模型

# 5. 定义模型
gru_model = build_gru(input_shape=(X_scaled.shape[1], 1))  # 创建GRU网络模型，输入形状为3D
gru_model.summary()  # 打印模型结构

# 6. 编译模型
gru_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.001), loss='mse', metrics=['mae'])  # 编译模型，使用Adam优化器，损失函数为MSE

# 7. 训练GRU网络
history_gru = gru_model.fit(
    X_train, Y_train,  # 训练数据和标签
    validation_data=(X_test, Y_test),  # 验证数据和标签
    epochs=2000,  # 训练100个轮次
    batch_size=64,  # 批次大小为32
    verbose=1  # 输出详细训练信息
)

# 8. 模型评估
# 进行预测
Y_train_pred = gru_model.predict(X_train).flatten()  # 训练集预测，并将预测结果展平成1D
Y_test_pred = gru_model.predict(X_test).flatten()  # 测试集预测，并将预测结果展平成1D
