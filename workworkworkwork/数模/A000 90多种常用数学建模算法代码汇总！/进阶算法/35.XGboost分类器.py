...
xgb_classifier = xgb.XGBClassifier(
    objective='binary:logistic',  # 二分类任务，使用logistic损失函数
...
# 4. 训练XGBoost分类器
xgb_classifier.fit(X_train, Y_train)  # 使用训练集训练模型

# 5. 模型评估与预测
Y_train_pred = xgb_classifier.predict(X_train)  # 对训练集进行预测
Y_test_pred = xgb_classifier.predict(X_test)  # 对测试集进行预测
