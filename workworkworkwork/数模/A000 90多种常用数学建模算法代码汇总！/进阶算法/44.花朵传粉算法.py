...
from mealpy.evolutionary_based.FPA import OriginalFPA as FPA  # 导入花朵传粉算法（Flower Pollination Algorithm, FPA）
from mealpy.utils.space import FloatVar  # 用于定义搜索空间

# 定义目标函数和惩罚函数
def objective_function(solution):
    """
    目标函数定义为三个变量的乘积，包含两个约束条件。
    """
    # 目标函数：三个变量相乘
...
# 定义搜索空间
new_bounds = [(-5, 50), (-10, 100), (-20, 200)]  # 定义每个变量的搜索范围
...
# 使用花朵传粉算法 (Flower Pollination Algorithm, FPA) 进行优化
optimizer = FPA(epoch=500, pop_size=100)  # 设置迭代次数为500次，种群大小为100个个体
best_solution = optimizer.solve(problem_dict)  # 调用solve函数，执行优化
...
def plot_history(history):
    ...
    plt.title("FPA Iteration Curve")  # 设置图表标题
...
# 打印最优解和对应的目标值
print(f"FPA Global Best Solution: {results['Solution']}, Global Best Fitness: {results['Fitness']}")
