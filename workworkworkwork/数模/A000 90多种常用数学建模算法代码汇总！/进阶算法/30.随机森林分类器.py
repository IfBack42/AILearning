...
# 3. 构建随机森林分类器
random_forest = RandomForestClassifier(n_estimators=100, random_state=42)  # 构建随机森林分类器，设定100棵树

# 4. 训练模型
random_forest.fit(X_train, Y_train)  # 使用训练集训练模型

# 5. 模型评估与预测
Y_train_pred = random_forest.predict(X_train)  # 对训练集进行预测
Y_test_pred = random_forest.predict(X_test)  # 对测试集进行预测
...
# 7. 绘制重要特征图
importances = random_forest.feature_importances_  # 获取特征的重要性分数
