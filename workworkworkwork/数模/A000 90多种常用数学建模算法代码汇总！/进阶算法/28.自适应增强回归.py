...
from sklearn.ensemble import AdaBoostRegressor  # 引入自适应增强回归模型
from sklearn.tree import DecisionTreeRegressor  # 作为弱学习器使用的决策树回归
...
# 3. 构建自适应增强回归模型
# 使用决策树回归作为弱学习器
weak_learner = DecisionTreeRegressor(max_depth=4)  # 设置最大深度为4的决策树
# 构建AdaBoost回归模型，使用50个弱学习器
...
# 5. 模型评估与预测
Y_train_pred = adaboost_regressor.predict(X_train)  # 训练集预测
Y_test_pred = adaboost_regressor.predict(X_test)  # 测试集预测
