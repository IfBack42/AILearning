...
from sklearn.linear_model import BayesianRidge  # 贝叶斯线性回归模型
...
# 3. 构建贝叶斯线性回归模型
bayesian_model = BayesianRidge()  # 使用BayesianRidge类创建贝叶斯线性回归模型

# 4. 训练模型
bayesian_model.fit(X_train, Y_train)

# 5. 模型评估与预测
Y_train_pred = bayesian_model.predict(X_train)  # 训练集预测
Y_test_pred = bayesian_model.predict(X_test)  # 测试集预测
