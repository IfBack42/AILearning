...
# 3. 构建CatBoost分类器（用于多分类任务）
catboost_classifier = CatBoostClassifier(
    iterations=100,  # 迭代次数，即树的数量
    learning_rate=0.1,  # 学习率
    depth=6,  # 树的最大深度
    loss_function='MultiClass',  # 多分类任务使用MultiClass损失
    verbose=10,  # 每10次迭代输出一次训练信息
    random_seed=42  # 随机种子
)

# 4. 训练CatBoost分类器
catboost_classifier.fit(X_train, Y_train, eval_set=(X_test, Y_test), plot=True)  # 使用训练集训练模型，并在测试集上进行验证

# 5. 模型评估与预测
Y_train_pred = catboost_classifier.predict(X_train)  # 对训练集进行预测
Y_test_pred = catboost_classifier.predict(X_test)  # 对测试集进行预测
