...
from mealpy.bio_based.SOS import OriginalSOS as SOS  # 只导入共生生物搜索算法（SOS）
...
# 使用共生生物搜索算法（SOS）进行优化
optimizer = SOS(epoch=500, pop_size=100)  # 设置迭代次数和种群大小
...
def plot_history(history):
    ...
    plt.title("SOS Iteration Curve")  # 设置标题
...
# 打印最优解和对应的目标值
print(f"SOS Global Best Solution: {results['Solution']}, Global Best Fitness: {results['Fitness']}")
