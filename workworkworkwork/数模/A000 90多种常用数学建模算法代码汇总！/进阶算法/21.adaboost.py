...
X_scaled = scaler.fit_transform(X)  # 对输入特征进行标准化
...
# 3. 构建 AdaBoost 回归模型
# 使用决策树作为基模型
base_estimator = DecisionTreeRegressor(max_depth=4)  # 基模型选择为深度为4的决策树
adaboost_model = AdaBoostRegressor(estimator=base_estimator, n_estimators=100, learning_rate=0.1, random_state=42)

# 4. 训练 AdaBoost 模型
adaboost_model.fit(X_train, Y_train)

# 5. 模型评估与预测
Y_train_pred = adaboost_model.predict(X_train)  # 训练集预测
Y_test_pred = adaboost_model.predict(X_test)  # 测试集预测
