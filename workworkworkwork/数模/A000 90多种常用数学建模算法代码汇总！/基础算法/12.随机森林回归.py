...
plt.title('Optimized Random Forest Regression Prediction on Training Set')
plt.xlabel('Index')
...
plt.title('Optimized Random Forest Regression Prediction on Test Set')
plt.xlabel('Index')
plt.ylabel('Target Value')
plt.legend()
plt.grid(True)
...
# 使用最佳模型进行预测
new_predictions = best_rf.predict(new_data_scaled)

# 保存预测结果到新的Excel文件
output_path = 'D:/py/LearnPython/predictions_rf.xlsx'
