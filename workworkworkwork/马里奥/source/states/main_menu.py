...
class MainMenu:
    def __init__(self,sound_manager):
        ...
        self.sound_manager.play_music('main_theme')  # 初始化后再调用
        self.start({'score': 0, 'coin': 0, 'lives': 3, 'time': C.LEVEL_TIME, 'player_state': 'small'})
    ...
    def update_cursor(self,keys):
        if keys[pygame.K_RETURN]:
            self.sound_manager.play_sfx('coin')  # 选择确认音效
        if keys[pygame.K_UP]:
    ...
    def reset_game_info(self): # 保留必要字段
        # 修改点：显式设置初始生命值
        self.game_info.update({
            'score': 0,
            'coin': 0,
            'lives': 3,  # 确保每次新游戏都从3条命开始
            'time': C.LEVEL_TIME,
            'player_state': 'small'
        })