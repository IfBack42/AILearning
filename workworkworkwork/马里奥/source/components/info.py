...
class Info:
    ...
    def create_state_labels(self):
        ...
        if self.state=='main_menu':
            self.state_labels.append((self.create_label('NO PEOPLE WITH YOU'),(272,360)))
            ...
            self.state_labels.append((self.create_label('CAI JIU DUO LIAN'), (240, 300)))

    def create_info_labels(self):
        self.info_labels = []
        self.score_label = self.create_label('000000')
        self.time_label = self.create_label(str(C.LEVEL_TIME))
        ...
        self.info_labels.append((self.score_label, (75, 55)))
        self.info_labels.append((self.create_label('x{:02d}'.format(self.game_info['coin'])), (300, 55)))
        self.info_labels.append((self.create_label('1 - 1'), (480, 55)))
        self.info_labels.append((self.time_label, (625, 55)))
    ...
    def update(self):
        ...
        self.game_info['time'] = max(C.LEVEL_TIME - elapsed, 0)

        # 修改点：动态更新分数标签
        self.score_label = self.create_label(f"{self.game_info['score']:06d}")
        self.time_label = self.create_label(f"{self.game_info['time']:03d}")

        # 重建标签列表
        self.info_labels = [
            (self.create_label('MARIO'), (75, 30)),
            (self.create_label('WORLD'), (450, 30)),
            (self.create_label('TIME'), (625, 30)),
            (self.score_label, (75, 55)),
            (self.create_label(f"x{self.game_info['coin']:02d}"), (300, 55)),
            (self.create_label('1 - 1'), (480, 55)),
            (self.time_label, (625, 55))
        ]
